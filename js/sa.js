window.sib.client_key !== '' && (function() {
    "use strict";
    var config = {
            ...
            cookie_exp: 182 * 24,
            if_url: 'https://sibautomation.com/cm.html?key=' + window.sib.client_key,
            is_secure: 'false',
    ...
    var iframe,cm;
    function q() {
        while (window.sib.equeue.length) { (function(obj) { for (var k in obj) { if(typeof window.sib[k] === "function") { setTimeout(function() { if(typeof window.sib[k] === "function") { window.sib[k](obj[k][0], obj[k][1], obj[k][2], obj[k][3]); } }, 200); } } })(window.sib.equeue.shift());}
    }
    function mo(t) {
    ...
    function br(d) {
        var td = { key: window.sib.client_key, cuid: Helper.cookie.get('sib_cuid'), ma_url: window.location.href }; window.sib.email_id && (td.email_id = window.sib.email_id); return mo(td, d);
    }
    function generateReqBody(event_type) {
        ...
        if (window.sib.email_id) 
            b.email_id = window.sib.email_id;
        return b;
    }
    function generateReqHeaders() {
        return { "Content-Type": "application/json;charset=UTF-8", "visitor_id": Helper.cookie.get('sib_cuid'), "ma-key": window.sib.client_key };
    }
    ...
    function load_cm(c){
        ...
        if (cuid && (contact_email || contact_id) && config.custom_url_parsing == "true"){
            ...
            const session_url = config.base_url + '/visitor/' + window.sib.client_key, x = new XMLHttpRequest(); x.open("POST", session_url, !0); x.setRequestHeader("Content-Type", "application/json;charset=UTF-8"); x.send(JSON.stringify(payload));
            return c && c();
        }
    ...
    load_cm( function () { q(); typeof create_chat == "function" && create_chat(); typeof triggerNotifyEngine == "function" && triggerNotifyEngine(); });
